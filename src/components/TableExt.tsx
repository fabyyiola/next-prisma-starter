...
export default function TableExt({
  ...
  const handleSearch = (query: string, columnIndex?: number) => {
  ...
  const filterRows = (query?: string, columnIndex?: number) => {
    ...
    if (query && columnIndex !== undefined) {
        ...
        filteredRowsCopy = initialTableRows.filter(row => {
            return row.cells.some(cell => cell.value.toLowerCase().includes(query.toLowerCase()));
        });
...
const handleOnTabClick = (columnName:string, filtervalue:string)=>{
          ...
          {showAddRecordButton && (
            <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
              <Button
                ...
                size="sm"
              >
                <UserPlusIcon strokeWidth={2} className="h-4 w-4" /> {addRecordButtonText}
              </Button>
            </div>
          )}
        </div>
        <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
          <Tabs value={activeTabValue} className="w-full md:w-max" onChange={handleTabChange}>
            <TabsHeader>
              {[allTab, ...tabs].map(({ label, value, columnIndex }) => (
                <Tab onClick={()=>{handleSearch(value,columnIndex)}} key={value} value={value}>
                  &nbsp;&nbsp;{label}&nbsp;&nbsp;
                </Tab>
              ))}
            </TabsHeader>
